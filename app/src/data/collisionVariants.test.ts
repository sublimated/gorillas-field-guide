...
import { collisionVariantFor, notationSignature } from './collisionVariants';
...
describe('collision variants', () => {
  ...
  it('marks known full-notation collisions', () => {
    ...
    const variant = collisionVariantFor(barkskin, 2);
  ...
  it('uses quiet spoken diphthongs instead of visible numeric variants', () => {
    const barkskin = collisionVariantFor(spell('barkskin'), 2);
    const enhanceAbility = collisionVariantFor(spell('enhance-ability'), 2);
  ...
  it('does not mark unique spell/cast signatures', () => {
    expect(collisionVariantFor(spell('acid-splash'), 3)).toBeNull();
  });

  it('uses the cast slot in the collision signature', () => {
    const barkskin = spell('barkskin');

    expect(collisionVariantFor(barkskin, 2)?.signature).toBe(notationSignature(barkskin, 2));
    expect(collisionVariantFor(barkskin, 3)?.signature).toBe(notationSignature(barkskin, 3));
  });
});
